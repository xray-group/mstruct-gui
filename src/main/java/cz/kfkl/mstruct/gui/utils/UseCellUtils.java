...
public class UseCellUtils {
	...
	static <T> ChoiceBox<T> createChoiceBox(final Cell<T> cell, final ObservableList<T> items,
...
//			event.consume();
	...
	static <T> ComboBox<T> createComboBox(final Cell<T> cell, final ObservableList<T> items,
		...
		// First listener attempts to commit or cancel when the ENTER or ESC keys are
		// released.
		...
		// Third listener makes an assumption about the skin being used, and attempts to
		// add
		// a listener to the ListView within it, such that when the user mouse clicks on
		// a
		// on an item, that is immediately committed and the cell exits the editing
		// mode.
