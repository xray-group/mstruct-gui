 ...
 * on the cell). A side-effect of this is that the usual editing callbacks (such
 * as {@link javafx.scene.control.TreeTableColumn#onEditCommitProperty() on edit
 * commit}) will <strong>not</strong> be called. If you want to be notified of
 * changes, it is recommended to directly observe the boolean properties that
 * are manipulated by the CheckBox.
 * </p>
 *
...
public class CheckBoxTreeTableCell<S, T> extends TreeTableCell<S, T> {
	 ...
	 *
	 * <p>
	 * When used in a TreeTableColumn, the CheckBoxCell is rendered with a CheckBox
	 * centered in the column.
	 *
	 * <p>
	 * The {@code ObservableValue<Boolean>} contained within each cell in the column
	 * will be bound bidirectionally. This means that the CheckBox in the cell will
	 ...
	 *         able to work on the type of element contained within the
	 *         TreeTableColumn.
	 */
	 ...
	 *                            {@link TreeTableColumn} instance.
	 * @param getSelectedProperty A Callback that, given an object of type
	 *                            {@code TreeTableColumn<S,T>}, will return an
	 *                            {@code ObservableValue<Boolean>} that represents
	 *                            whether the given item is selected or not. This
	 *                            {@code ObservableValue<Boolean>} will be bound
	 *                            bidirectionally (meaning that the CheckBox in the
	 *                            cell will set/unset this property based on user
	 *                            interactions, and the CheckBox will reflect the
	 *                            state of the {@code ObservableValue<Boolean>}, if
	 ...
	 *                            state of the {@code ObservableValue<Boolean>}, if
	 *                            it changes externally).
	 * @param showLabel           In some cases, it may be desirable to show a label
	 *                            in the TreeTableCell beside the {@link CheckBox}.
	 *                            By default a label is not shown, but by setting
	 *                            this to true the item in the cell will also have
	 *                            toString() called on it. If this is not the
	 *                            desired behavior, consider using
	 *                            {@link #forTreeTableColumn(javafx.util.Callback, javafx.util.StringConverter) },
	 *                            which allows for you to provide a callback that
	 *                            specifies the label for a given row item.
	 * @return A {@link Callback} that will return a {@link TreeTableCell} that is
	 *         able to work on the type of element contained within the
	 *         TreeTableColumn.
	 */
	public static <S, T> Callback<TreeTableColumn<S, T>, TreeTableCell<S, T>> forTreeTableColumn(
			final Callback<Integer, ObservableValue<Boolean>> getSelectedProperty, final boolean showLabel) {
		StringConverter<T> converter = !showLabel ? null : UseCellUtils.<T>defaultStringConverter();
	 ...
	 *                            state of the {@code ObservableValue<Boolean>}, if
	 *                            it changes externally).
	 * @param converter           A StringConverter that, give an object of type T,
	 *                            will return a String that can be used to represent
	 *                            the object visually. The default implementation in
	 *                            {@link #forTreeTableColumn(Callback, boolean)}
	 *                            (when showLabel is true) is to simply call
	 *                            .toString() on all non-null items (and to just
	 *                            return an empty string in cases where the given
	 *                            item is null).
	 * @return A {@link Callback} that will return a {@link TreeTableCell} that is
	 *         able to work on the type of element contained within the
	 *         TreeTableColumn.
	 */
	public static <S, T> Callback<TreeTableColumn<S, T>, TreeTableCell<S, T>> forTreeTableColumn(
			final Callback<Integer, ObservableValue<Boolean>> getSelectedProperty, final StringConverter<T> converter) {
	 ...
	 *                            {@link ObservableValue} given an index from the
	 *                            TreeTableColumn.
	 */
	public CheckBoxTreeTableCell(final Callback<Integer, ObservableValue<Boolean>> getSelectedProperty) {
	 ...
	 *
	 * @param getSelectedProperty A {@link Callback} that will return a
	 *                            {@link ObservableValue} given an index from the
	 *                            TreeTableColumn.
	 * @param converter           A StringConverter that, given an object of type T,
	 *                            will return a String that can be used to represent
	 *                            the object visually.
	 */
	public CheckBoxTreeTableCell(final Callback<Integer, ObservableValue<Boolean>> getSelectedProperty,
...
//        prop.set(this, Pos.CENTER);
	 ...
	 * shown on screen.
	 * 
	 * @return the property representing the {@link Callback} that is bound to by
	 ...
	 * 
	 * @param value the {@link Callback} that is bound to by the CheckBox shown on
	 *              screen
	 */
	...
	public void updateItem(T item, boolean empty) {
		...
		if (empty) {
			...
			checkBox.disableProperty().bind(Bindings
					.not(getTreeTableView().editableProperty().and(getTableColumn().editableProperty()).and(editableProperty())));
		}
	}
